...
var _abanTimer = null;
function _scheduleAbandon() {
  clearTimeout(_abanTimer);
  _abanTimer = setTimeout(function() {
    var name  = (document.getElementById('codName')  || {}).value || '';
    var phone = (document.getElementById('codPhone') || {}).value || '';
    if (!name.trim() && !phone.trim()) return;
    var prod = (document.getElementById('codProdName') || {}).value || '';
    fetch('/save-abandoned.php', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ name: name.trim(), phone: phone.trim(), product: prod, page: window.location.pathname })
    }).catch(function(){});
  }, 30000); // 30 seconds after last keystroke
}
...
function _openModal(product, price, waMode) {
  ...
  // Attach abandon tracking to form fields
  setTimeout(function() {
    ['codName','codPhone','codCity','codAddr'].forEach(function(id) {
      var el = document.getElementById(id);
      if (el) el.addEventListener('input', _scheduleAbandon);
    });
  }, 100);
}
...
function codSubmit() {
  ...
  clearTimeout(_abanTimer); // order submitted — cancel abandon capture
